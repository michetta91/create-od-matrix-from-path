namespace AggregateZonesConnectedToTheSameNode
{
    internal class Exporter
    {
        internal static void ExportToNetFile(string outputFilePathNet, Maps maps)
        {
            ...
            {
                var connectedNodes = maps.MainZoneToElementMap[zoneToMainZone.Value].Split("-");
                foreach (var el in connectedNodes)
                {
                    myLines.Add($"{zoneToMainZone.Key};{el};O;9;");
                    myLines.Add($"{zoneToMainZone.Key};{el};D;9;");
                }
            }
